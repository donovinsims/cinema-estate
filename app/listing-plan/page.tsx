...
export default function ListingPlanPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      track("listing_plan_preview_generated", { readinessTier: result.score.totalScore >= 80 ? "Strong" : result.score.totalScore >= 60 ? "Solid" : result.score.totalScore >= 40 ? "Developing" : "Early", readinessScore: result.score.totalScore, cinemaEstateFit: result.cinemaEstateFit.strength, propertyType: dataRef.current.propertyType ?? "unknown" });
      setPreview(result);
      setPhase("preview");
    } catch (err: unknown) {
  ...
  const handleClaim = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    track("listing_plan_claim_attempted");
    ...
    try {
      ...
      track("listing_plan_claimed", { marketingConsent, cinemaEstateFit: plan.cinemaEstateFit.strength, readinessScore: plan.score.totalScore });
    ...
    return (
      <main className="lp-page">
        ...
        <div className="lp-entry section-shell">
          ...
          <button className="button button-primary lp-start-btn" onClick={() => { track("listing_plan_started"); setPhase("wizard"); }}>
  ...
  if (phase === "results" && fullPlan) {      const { score: rScore, cinemaEstateFit } = fullPlan;
