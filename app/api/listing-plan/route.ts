 ...
 * (https://docs.sequenzy.com/api-reference/transactional/send) using the provisioned
 * `listing-plan-delivery` template (slug: listing-plan-delivery). This is the transactional
 * delivery (not marketing) and runs on every `?action=generate` call regardless of
 * marketingConsent.
 */
async function sendTransactionalPlan(
  ...
  const topGaps = Array.isArray(plan.gaps) ? (plan.gaps as string[]).map((g) => ({ gap: g })) : [];
  const doNow = Array.isArray(plan.doNow) ? (plan.doNow as string[]).map((a) => ({ action: a })) : [];
  ...
  try {
    const res = await fetch(`${SEQUENZY_API_BASE}/transactional/send`, {
      ...
      body: JSON.stringify({
        to: email,
        templateId: "listing-plan-delivery",
        variables: {
          score,
          tier,
          readinessInterpretation:
            typeof plan.readinessInterpretation === "string" ? plan.readinessInterpretation : "",
          summary:
            typeof plan.sellerFacingSummary === "string" ? plan.sellerFacingSummary : "",
