...
- **Pricing section redesign — MERGED (2026-08-07):** PR **[#16](https://github.com/donovinsims/cinema-estate/pull/16)** rebuilt `.price-section`/`.tier-*` in `app/globals.css` (and a matching one-line JSX addition in `app/page.tsx`) into a dark glass-card layout inspired by a 21st.dev pricing component, replacing the earlier light-blue background/white cards. Merged by the owner into `main`.
- **Interface polish + CRO placement — MERGED (2026-08-07):** PR **[#18](https://github.com/donovinsims/cinema-estate/pull/18)** (branch `sales-page-refinement-round3-wt`, commit `0a448b2`) merged into `main` as `b04fa6c`. Ten scoped fixes: token consolidation (`--offwhite`), spacing standardized onto the 4/8/16/24/32 scale, two type-scale merges, `.button-dark` shadow, a `.quality-grid li` selector-leak fix (numbered `.how-it-works` steps via CSS counters, `.checklist` keeps its icon treatment), bare-arrow Unicode → shared `app/ArrowIcon.tsx` SVG, modal close animation, About copy condensed 5→3 paragraphs, objection-handling copy restated at the proof/pricing friction points, and consent-gated funnel analytics (scroll-depth, per-tier view/hover, FAQ-open). No checkout/pricing/route changes.
- **Pricing CTA polish — MERGED (2026-08-07):** PR **[#17](https://github.com/donovinsims/cinema-estate/pull/17)** (branch `sales-page-refinement-round3`, commits `2f52cfc` and `6403f3e`) merged into `main`. Two small follow-up commits on top of the PR #16 redesign, still in `app/globals.css`'s `.tier-*` rules only:
...
- **PR2 — cinematic editorial redesign + Villa Siena demo integration — COMPLETE, PUSHED, PR OPEN FOR REVIEW (2026-08-07).** Branch `sales-page/02-cinematic-editorial-redesign`, created off `main` at `0cd16f5`. PR **[#22](https://github.com/donovinsims/cinema-estate/pull/22)** is open against `main` — **not merged, per hard instruction; a future session must not merge it or start PR3 without the owner merging PR2 first.** This was a large hand-orchestrated (no `/sales-page-upgrade` skill, no `cinema-estate-*` subagents, per explicit owner instruction) pass covering the remaining PR1-shaped conversion gaps (folded into this PR's start), the macro visual redesign, and — added mid-PR2, per owner instruction — replacing the self-produced "255 Eldon Ave" demo with **Villa Siena** (607 Siena Way, Bel-Air) as the primary on-site demo. `npm run lint` (0 errors), `npm test` (15/15), `npm run build`, `npx next build`, and `git diff --check` all pass at the tip.
  - **Tasks 0-3 (design tokens, hero restructure, comparison chrome, early-access reframe):** done in earlier checkpoints on this branch — see the detailed task log below, unchanged from prior sessions' work.
  - **Villa Siena demo integration — done.** New route `app/villa-siena/page.tsx` (Server Component, exports its own `metadata`/canonical) + `app/villa-siena/RevealObserver.tsx` (an isolated Client Component for the scroll-reveal animation) + `app/villa-siena/villa-siena.module.css` (this repo's first CSS Module, scoped so the route's own bronze/moss/paper Newsreader-serif art direction never touches Cinema Estate's ink/bone/tungsten tokens) — built by a worktree-isolated subagent, reviewed and integrated by the lead session. All 12 source files (6 photos, 5 videos, 1 narration WAV) copied byte-for-byte from `work/villa-siena-package/extracted/` into `public/media/villa-siena/`; the 11 manifest-listed files were verified against `manifest.json`'s SHA-256 list, and the WAV (not manifest-listed) was verified by comparing its own source/copy hashes directly. `/villa-siena` is registered in `app/sitemap.ts` and covered by `tests/villa-siena-route.test.mjs`, plus new assertions in `tests/seo-routes.test.mjs`/`tests/structured-data.test.mjs` confirming it appears in the sitemap and declares its own canonical link. On the homepage: the hero eyebrow/media, `ComparisonExperience`'s before/after pair (Great Room and Garden → Living With Nature, matching the property's own signature shot per the owner-supplied landing page), and a new `ProofReel.tsx` client component (one dominant player defaulting to the 32s narrated final film + a 5-clip scene strip, native `<video controls>` only) all now use real Villa Siena media. "Everything you get" links each deliverable to real evidence, including a `<audio>` narration sample honestly captioned as not frame-matched to the final film (the supplied WAV runs 48.05s per `ffprobe`, the film runs 32.005s — measured, not assumed) and a link to the new route ("View the Villa Siena property page"). About's paragraph-3 demo disclosure was rewritten: Villa Siena is a real, permission-cleared listing used as a demonstration, its video/page produced by a separate production tool, not delivered as paid Cinema Estate client work — `docs/PRODUCT.md` and `.claude/skills/sales-page-upgrade/references/product-guardrails.md`'s "Demo disclosure requirement" updated to match. Eldon's 8 media files remain in `public/media/` but are no longer referenced anywhere in `app/`, `tests/`, or `docs/` (confirmed via repo-wide grep) — left in place rather than deleted, to keep this PR's diff bounded; a future session can remove them once confirmed unneeded.
  - **Tasks 4-9 (remaining PR2 visual tasks) — done.** Task 4/5: proof/package sections rebuilt around the `ProofReel` component and real Villa Siena evidence (see above — this also resolved the earlier "no real narration/listing-page asset" open question, since Villa Siena has real assets for all four deliverables). Task 6: pricing — removed the recommended tier's blue glow halo (both the desktop rule and the separate mobile-media-query rule) and the glossy blue-gradient `.tier-cta`, retinted to tungsten, Story now appears first on mobile via `.tier-card.is-recommended { order: -1 }` inside the existing 720px media query. Task 7: About pull-quote (an inline `<span class="about-pullquote">` around the positioning sentence, styled in the Fraunces display font — kept the test's paragraph-count/structure assertion intact by updating its expected string to include the new markup) and FAQ chrome (tungsten `+`/`−` markers, a CSS-counter leading-zero numbering matching `.how-it-works`' pattern, opened-question serif emphasis — all CSS-only, `<details>`/`<summary>` semantics untouched). Task 8: `.waitlist-section .eyebrow`'s hardcoded `#4864c5` replaced with `var(--tungsten)`. Task 9: swept the remaining decorative `var(--blue)`/`var(--blue-light)` uses (tier flag/checkmarks, how-it-works step markers, checklist icons, price-section accents, modal-close hover, waitlist-form-modal button, checkout-status success border, analytics-consent accept button, mobile comparison-toggle active state) to tungsten, leaving genuine hyperlinks (terms/privacy links, guarantee line, checkout-status/analytics-consent anchors) and focus-visible outlines on the site's own quiet `--link` blue per CLAUDE.md's documented carve-out; also removed now-dead `.proof-grid`/`.proof-card` CSS left over from the pre-`ProofReel` markup.
  - **Verification — done.** Manually verified in-browser (working around this session's shared-browser-pane tooling flakiness — the same contention the Villa Siena worktree subagent also hit — via JS-driven navigation/scrolling and `get_page_text`/network-request introspection as reliable fallbacks alongside screenshots) at 375/768/1440px: homepage hero, comparison, proof reel, package section, pricing (Story first on mobile confirmed), About, FAQ, final CTA, and the `/villa-siena` route directly — correct responsive behavior at all three widths, no broken media requests (all 200/206/304, no 404s), no console errors, reduced-motion and reveal-on-scroll behavior intact. No real Polar checkout was performed (one accidental navigation into a live Polar checkout page occurred during scroll-testing; no fields were touched, navigated away immediately).
  - **Reconciliation with `origin/main`:** this branch predated `origin/main`'s `0cd16f5` ("Persist engineering-workflow requirement in CLAUDE.md (#21)"); merged it in cleanly (docs-only, no conflicts) at the start of this session, so `CLAUDE.md`'s "Engineering workflow" section and `tasks/todo.md`/`tasks/lessons.md` are now present on this branch.
  - **Post-verification fix (2026-08-07, after PR #22 was opened):** live-browser verification (computed-style + before/after screenshot, not just source review) found `--font-display` — the token `h1`/`h2` and the FAQ's opened-summary rule were meant to use for Fraunces — resolved to an empty string everywhere, including on `:root` itself, silently falling back to the body's Inter font for every heading site-wide. Root cause traced to how Tailwind v4's `@import "tailwindcss"` cascade layers interact with this file's unlayered `:root` block; direct `var(--font-fraunces)` references (confirmed via inline-style testing) resolve fine, ruling out the chained-`var()` pattern itself. Fixed by pointing `h1, h2` and `.answer-list details[open] summary` at `var(--font-fraunces)` directly and removing the now-dead `--font-display` token, rather than leave a broken alias for a future session to reuse. This was a genuine, pre-existing regression from Task 1 (marked "Done" in earlier checkpoints but never actually rendering correctly) — commit `cf2ebcc`, pushed as a follow-up to PR #22.
  - **The Vercel-build/stale-deploy incident (fixed on this branch, not yet on `main`):** production (`cinema-estate.vercel.app`) was serving a stale build missing everything from PR #19 onward. Root cause: `vercel.json` sets `"buildCommand": "npx next build"`, bypassing this project's real `vinext build` entirely — and `npx next build` runs a stricter TypeScript check than `vinext build` does. `app/EarlyAccessModal.tsx` imports `MouseEvent` from `"react"` for JSX handler typing, which shadowed the DOM `MouseEvent` global used by a `document.addEventListener("mouseout", ...)` listener (added in PR #19's exit-intent feature); `next build`'s typecheck caught the resulting type mismatch and every Vercel deploy since PR #19 merged has errored (confirmed via `vercel ls` / `vercel inspect --logs` in an earlier session). Fixed on this branch by retyping that one handler against `globalThis.MouseEvent`. **Not fixed, by explicit owner instruction:** the deeper mismatch of Vercel building this Cloudflare-Worker/vinext app via a bypassed plain `next build` at all (meaning production may not be running the real worker entry — image optimization route, etc. — never confirmed either way). This remains an owner infrastructure decision, not something to silently fix. Once this branch's fix merges to `main`, the next Vercel deploy should succeed and production should catch up — worth confirming with `vercel ls` after merge.

## Demo-listing swap: Eldon → Villa Siena — DONE (2026-08-07, as part of PR2, see above for the full summary)

The owner supplied a new demo package (`Villa-Siena-607-Siena-Way.zip`, originally in `~/Downloads`) to replace the current self-produced 255 Eldon Ave demo with a real listing: **Villa Siena, 607 Siena Way, Bel-Air, CA 90077**. The owner explicitly confirmed usage rights are cleared with the listing team (Michael Fahimian and Branden Williams, The Beverly Hills Estates) and is OK disclosing that the video/landing-page assets were produced with a different tool, not Cinema Estate's own process — see "the honesty requirement" below before writing any copy.

**The package is saved locally and persistently** (gitignored, not committed, survives across sessions on this machine — but does *not* survive a fresh clone/environment, so if a future session runs somewhere else, get the zip from the owner again) at:
- `work/villa-siena-package/Villa-Siena-607-Siena-Way.zip` (the original zip, untouched)
- `work/villa-siena-package/extracted/` (already unzipped, same structure)

### Explicit owner instruction on scope

**Do not touch, re-encode, crop, or otherwise edit any of the product videos or photos — use them exactly as delivered.** The only production work the owner asked for is **rebuilding `04-Landing-Page/Villa-Siena-Landing-Page.html` so it can be presented on the live site** (i.e., adapt it into a real route/page in this app, not just left as a standalone file). Read that HTML file before starting — as of this note it's a small, self-contained 60-line page: `<link>`s to Google Fonts (external, intentional per the package's own README) and to the official brokerage listing `https://thebeverlyhillsestates.com/listing/607-siena-way/` (external, intentional), plus 4 in-page anchors (`#residence`, `#film`, `#amenities`, `#inquire`) and relative `src`s to 3 of the 6 original photos (`02-Great-Room-and-Garden.jpg`, `03-Floating-Stair-and-Bronze-Wall.jpg`, `05-Primary-Bath.jpg`) plus the final film video. No HyperAgent-hosted references were found in it (matches the package's own README claim of "no HyperAgent name, logo, source URL, or media dependency"). Rebuilding it likely means: a new Next.js route (e.g. `app/demo/villa-siena/page.tsx` or similar — owner's call on the exact path) that reproduces this page's content/design using `next/image`/`<video>` pointing at wherever the photos/videos land under `public/media/`, keeping the two external links as-is.

### What's in the package (for reference, don't re-derive)

- `01-Listing-URL/Zillow-Listing-URL.txt` — the Zillow listing URL.
- `02-Original-Images/` — 6 authentic property photos (`01-Night-Aerial-Exterior.jpg` through `06-Stair-Lounge.jpg`) plus `Sources-and-Attribution.txt` (rights notes, source URLs, brokerage/agent names — read this before publishing anything, and re-confirm with the owner if anything here has changed).
- `03-New-Videos/` — `00-WATCH-THIS-FINAL-FILM.mp4` (32s narrated final film) plus 4 supporting 8-second scene clips (`01-Night-Arrival`, `02-Living-With-Nature`, `03-Floating-Stair`, `04-Fire-and-Water`) — these map naturally onto the current `proofClips` grid in `app/page.tsx`, same role the 4 Eldon scene clips play today.
- `04-Landing-Page/Villa-Siena-Landing-Page.html` — see above.
- `manifest.json` — every file's size + SHA-256, for verifying nothing changed in transit.
- `README.txt` — the package's own quick-start and branding notes.

### The honesty requirement — read before writing any demo copy

The current Eldon demo works because it's self-produced by the founder with clean rights, honestly disclosed as a demo (see `product-guardrails.md`'s "Demo disclosure requirement"). Villa Siena is materially different and the copy must say so honestly, not reuse the old "self-produced" framing:
- The **photos** are real listing photography from an active third-party listing, used with the listing team's permission (owner-confirmed, not independently verified by any agent session — if that permission status ever changes, this whole swap needs to be reverted or re-cleared).
- The **video and landing-page HTML** were generated by a different tool (the package's `manifest.json` shows a `published_landing_page_url` on `hyperagent.com`) — **not** produced by Cinema Estate's own process. This must be disclosed prominently (matching or exceeding the current About-section disclosure's prominence), not buried in a footnote, and must not be presented as "what Cinema Estate's process outputs."

### Narration audio — added 2026-08-07, after the initial package

The owner supplied a separate narration audio file, **`villa-siena-hero-narration.wav`** (mono, 24kHz, 16-bit PCM, ~2.3MB, ~2min based on file size at that rate), stating this is a standard output for this product (every landing-page delivery includes an AI-voice narration audio file) and that it belongs somewhere on the landing page. It's saved at:
- `work/villa-siena-package/extracted/05-Narration-Audio/villa-siena-hero-narration.wav`

**Resolved (owner confirmed with a screenshot 2026-08-07):** there's no separate `<audio>` element anywhere in `Villa-Siena-Landing-Page.html` as delivered — the "audio placeholder" the owner meant is the native control bar's mute/unmute icon on the hero `<video>` (`00-WATCH-THIS-FINAL-FILM.mp4`, the 32s final film). So `villa-siena-hero-narration.wav` is this hero video's narration track. When rebuilding the landing page (see "Explicit owner instruction on scope" above), wire it in as that video's audio — i.e., build the hero exactly like the current site's `HeroVideo` pattern (a real `<video>` with native/custom controls including mute/unmute, not a second, separate `<audio>` element). **Checked with `ffprobe` (2026-08-07):** `00-WATCH-THIS-FINAL-FILM.mp4` already has its own 32.005s stereo AAC audio track muxed in — matching the package's own description of it as "the completed campaign film **with narration**." **Resolved with measurement (2026-08-07, later in the same day, during PR2's Villa Siena integration):** the standalone `villa-siena-hero-narration.wav` is 48.05s (`ffprobe`), not 32.005s — **they do not match**, so the WAV is not a frame-accurate isolated cut of the film's own voiceover track. It's used on the homepage's "Everything you get" section as a narration-sample `<audio>` element, captioned honestly as a sample rather than claimed to be frame-matched to the film — this was a real, measured fact worth getting right in copy, not a case that needed asking the owner (the guardrails already required honest disclosure; the measurement just supplied the fact). No audio engineering was performed either way — both files are used exactly as supplied.

### Full list of files that reference the Eldon demo and will need updating

Found via `grep -rl "Eldon" app tests docs .claude/skills HANDOFF.md CLAUDE.md`: `app/ComparisonExperience.tsx`, `app/page.tsx` (hero eyebrow, proof clips, about copy, footer/media paths), `tests/rendered-html.test.mjs`, `docs/PRODUCT.md`, `HANDOFF.md` (this file, historical entries — leave those as history, just don't let them contradict the "current state"), and `.claude/skills/sales-page-upgrade/references/product-guardrails.md`'s "Demo disclosure requirement" section (this is the permanent factual reference other future agent sessions read before touching demo copy — it must be updated to describe Villa Siena's actual facts, or a future session will incorrectly re-assert the old Eldon/self-produced framing). `app/StructuredData.tsx`/`app/product-data.mjs` don't reference Eldon by name (they only carry tier pricing), so no change needed there.

## PR2 — cinematic editorial redesign — detailed task log (DONE)

Full plan: this run followed a plan approved by the owner across several rounds of `AskUserQuestion`/`ExitPlanMode` review in the planning session, saved at (session-local, not in this repo) `~/.claude/plans/you-are-the-senior-reactive-emerson.md`. The short version, since that file isn't part of this checkout: the owner's original mega-brief asked for a 3-PR rebuild (PR1 conversion architecture, PR2 macro visual redesign, PR3 component/interaction polish); research found PR1's content mostly already merged across ~20 prior PRs, so the owner had this session fold the small remaining PR1-shaped gaps (early-access identity conflict, "most listings" claim, thin post-purchase clarity, undersold process copy) into the *start* of PR2 rather than opening a separate PR1, then proceed with the macro visual redesign as PR2 itself. PR3 (button/icon/shape system finalization, full motion language, `make-interfaces-feel-better`'s aggressive micro-interaction pass, responsive/a11y/perf QA) is explicitly deferred to a future session, after this PR merges.

**Non-negotiable, regardless of anything the original brief said about auto-merging or auto-chaining phases:** this PR must stay open, unmerged, for the owner to review — matching `CLAUDE.md` and `.claude/skills/sales-page-upgrade/SKILL.md`'s own hard rule. No future session should merge this PR or start PR3 without the owner merging PR2 first.

### Task-by-task status

| # | Task | Status |
|---|---|---|
| 0 | Vercel build hotfix (`MouseEvent` type shadow in `EarlyAccessModal.tsx`) | Done |
| 0b(a) | Early-access → truthful "ask a question" reframe, exit-intent-vs-CTA check | Done |
| 0b(b) | "Most listings choose this" → "Recommended" | Done |
| 0b(c) | Post-purchase clarity (expanded checkout banner + on-page process) | Done |
| 0b(d) | 3-step → 4-step process, accurate 24h-trigger line | Done |
| 1 | Design tokens: warm ink/bone palette, tungsten accent, radius scale, Fraunces | Done |
| 2 | Hero restructure (message-left/framed-media-right), hero claim fix | Done |
| 3 | Comparison section chrome re-skin (logic/a11y/reduced-motion untouched) | Done |
| 4 | "Everything you get" section visual upgrade | **Done** — rebuilt around real Villa Siena evidence per deliverable |
| 5 | Proof-media consolidation (one primary player + scene strip) | **Done** — new `ProofReel.tsx` |
| 6 | Pricing composition rebuild (remove gradient CTA + glow halo) | **Done** |
| 7 | About re-skin + FAQ accordion chrome (FAQ *content* already done in 0b) | **Done** |
| 8 | Final-CTA chrome re-skin (copy/logic already done in 0b(a); header nav already done in Task 2) | **Done** |
| 9 | Sweep remaining generic-SaaS tells | **Done** |
| — | Villa Siena demo swap (route + homepage integration) | **Done** — `/villa-siena`, see "Demo-listing swap" section above |
| — | Verification pass (browser 375/768/1440, screenshots) | **Done** |
| — | Open PR against `main` | **Done — [PR #22](https://github.com/donovinsims/cinema-estate/pull/22), open, not merged** |

### Files touched (all committed and pushed)

Tasks 0-3 (prior checkpoints): `app/EarlyAccessModal.tsx`, `app/EarlyAccessButton.tsx`, `app/early-access-copy.mjs`, `app/WaitlistForm.tsx`, `app/api/early-access/route.ts`, `app/privacy/page.tsx`, `app/AnalyticsConsent.tsx`, `app/CheckoutStatus.tsx`, `app/AnswersAccordion.tsx`, `app/StructuredData.tsx`, `app/ComparisonExperience.tsx` (`id="transformation"`), `app/layout.tsx` (Fraunces font, theme-color).

Tasks 4-9 + Villa Siena demo swap (this checkpoint): `app/page.tsx` (hero, proof/package sections, pricing, About paragraph 3), `app/ComparisonExperience.tsx` (media swap only, interaction logic untouched), `app/globals.css` (proof-reel/deliverable-evidence rules, tier-* rebuild, About pull-quote, FAQ chrome, waitlist-section fix, blue→tungsten sweep), new `app/ProofReel.tsx`, `docs/PRODUCT.md`, `.claude/skills/sales-page-upgrade/references/product-guardrails.md`, `tests/rendered-html.test.mjs`, `tests/seo-routes.test.mjs`, `tests/structured-data.test.mjs`. New (worktree-built, integrated): `app/villa-siena/page.tsx`, `app/villa-siena/RevealObserver.tsx`, `app/villa-siena/villa-siena.module.css`, `app/sitemap.ts` (added `/villa-siena`), `tests/villa-siena-route.test.mjs`, `public/media/villa-siena/*` (12 files).

Untouched, as planned: `app/CheckoutButton.tsx`, `app/TierImpressionTracker.tsx`, `app/ScrollDepthTracker.tsx`, checkout/pricing logic, `app/AnswersAccordion.tsx`'s JSX (chrome was CSS-only).

### To resume (if the owner requests changes before merging)

PR2 itself is complete and open for review — nothing left to resume. If the owner requests
changes on [PR #22](https://github.com/donovinsims/cinema-estate/pull/22) before merging, push
follow-up commits directly to `sales-page/02-cinematic-editorial-redesign`. Do not merge PR2 or
start PR3 without the owner merging PR2 first.
