...
- **Pricing CTA polish — on branch `sales-page-refinement-round3`, commits `2f52cfc` and `6403f3e` (2026-08-07), NOT a mistake, do not revert:** two small follow-up commits on top of the PR #16 redesign, still in `app/globals.css`'s `.tier-*` rules only:
  ...
  - If another session sees these `.tier-*` values differ from what PR #16 originally shipped, that's expected — these are two intentional, owner-requested follow-ups, verified with `npm run lint` + `npm test` (12/12) + `git diff --check` after each, not drift to fix.
