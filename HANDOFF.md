...
- **In progress, not live:** a `/sales-page-upgrade full` run is underway on branch `codex/sales-page-upgrade-2026-08-06` (uncommitted working-tree changes, `main` itself is untouched). Paused mid-Phase-3 for session quota — see "Sales-page-upgrade workflow — 2026-08-06 (in progress, paused for quota)" below for the exact resume point before doing anything else in this repo.

## Sales-page-upgrade workflow — 2026-08-06 (in progress, paused for quota)

Running `/sales-page-upgrade full` per `.claude/skills/sales-page-upgrade/SKILL.md`. Paused mid-Phase-3 at the user's request (session was at 83% of its 5-hour quota) — **nothing is lost**, this section is the exact resume point. Branch: `codex/sales-page-upgrade-2026-08-06` (created off `main` at `7d86d40`, not pushed, not merged). **Nothing in this section has been committed** — it's all live working-tree changes; run `git status --short` and compare against the file list below before trusting anything here.
...
1. Lead integrates every delegated worktree's diff into `codex/sales-page-upgrade-2026-08-06`, manually adds the 4th FAQ item to the `StructuredData` `FAQPage` list, resolves the `CheckoutStatus` import now that the file exists.
2. Run `npm ci --include=dev`, `npm run lint`, `npm test`, `git diff --check` — **none of this has been run yet** against the current changes; the build is currently known-broken because of the unresolved `CheckoutStatus` import.
3. Phase 4 — `cinema-estate-copy-finisher` pass (not started).
